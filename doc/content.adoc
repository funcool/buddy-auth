...
[source,groovy]
----
compile "buddy:buddy-auth:0.6.0"
----
...
You can see a complete example of using this backend
link:https://github.com/funcool/buddy-auth/tree/master/examples/token-jws[here] (also with
authorization).
...
The main difference is that the backend uses JWE (Json Web Encryption) instead of JWS (Json Web Signature)
and it has the advangage that the content of token is encrypted instead of simply signed. This is useful
when token may contain some additional user information that can not be public.

This is how will look the previous (jws) example but using jwe with asymetric key encryption
algorithm:
...
this link:https://funcool.github.io/buddy-sign/latest/#generate-keypairs[faq entry].

You can see a complete example of using this backend
link:https://github.com/funcool/buddy-auth/tree/master/examples/token-jwe[here] (not related to
