...
In contrast to the vast majority of authentication libraries that I know that
_buddy_ does not mix authentication process with the authorization.

It is implemented as plugable backends that can be picked as is or implemented
new ones with simple steps. This is a list of builtin backends:

.List of builtin backends.
[options="header"]
|================================================
| Backend name | Namespace
| Http Basic   | `buddy.auth.backends/http-basic`
| Session      | `buddy.auth.backends/session`
| Token        | `buddy.auth.backends/token`
| JWS Token    | `buddy.auth.backends/jws`
| JWE Token    | `buddy.auth.backends/jwe`
|================================================

If you are not happy with the built-in backends, you can implement your own and
use it with _buddy-auth_ middleware without any problem.

The authentication process works mainly in two steps:

1. *parse*: that is responsible of analyze the request and read the auth related
   data (e.g. `Authorization` header, url params, etc..)
2. *auth*: with the data obtained from parse step just try to authenticate the
   the request (e.g. simple access to database for obtain the possible user, unsing
   self contained jws/jwe token, check a key in the session, etc...)

This step does not raises any exception and is completly transparent to the user.
The authentication process responsability is determine if request is anonymous or
is authenticated, nothing more.
...
The basic step for check if a request is authenticated or not, is just check if
it comes with `:identity` key and it contains a logical `true` (exists and contains
something different to `nil` or `false`).
...
(def backend (backends/http-basic {:realm "MyApi"
                                   :authfn my-authfn}))
----

The `authfn` is the responsible of the second step of authentication. It receives
...
From now, all requests that reaches `my-handler` will be properly authenticated.
...
Is a backend that uses tokens for authenticating the user. It behaves very
...
(def backend (backends/http-basic
              {:realm "API"
               :authfn my-auth-fn
